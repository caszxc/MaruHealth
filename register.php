            ...
            if ($stmt->rowCount() > 0) {
                ...
                if (!move_uploaded_file($_FILES["validID_front"]["tmp_name"], $validIdFrontPath)) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO pending_users 
                        (first_name, last_name, middle_name, gender, birthday, address, email, phone_number, valid_id_front, password, family_number, date_registered) 
                        VALUES 
                        (:firstName, :lastName, :middleName, :gender, :birthday, :address, :email, :phone, :validIDFront, :password, :familyNumber, NOW())");
...
<body>
                    ...
                    <div class="form-step active">
                        ...
                        <div class="group-col">
                            <label class="checkbox-label">
                                <input type="checkbox" name="hasFamilyNumber" id="hasFamilyNumber" <?= isset($_POST['hasFamilyNumber']) && $_POST['hasFamilyNumber'] === 'on' ? 'checked' : '' ?>>
                                I know my family number
                            </label>
                        </div>

                        <div class="group-col" id="familyNumberContainer" style="display: <?= isset($_POST['hasFamilyNumber']) && $_POST['hasFamilyNumber'] === 'on' ? 'flex' : 'none' ?>;">
                            <label>Family Number</label>
                            <input type="text" name="familyNumber" id="familyNumber" value="<?= htmlspecialchars($_POST['familyNumber'] ?? '') ?>">
                            <small class="field-hint">Enter your family number for record linking (letters, numbers, and hyphens only)</small>
                        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // Family Number Elements
            const hasFamilyNumberCheckbox = document.getElementById('hasFamilyNumber');
            const familyNumberContainer = document.getElementById('familyNumberContainer');
            ...
            const formFields = {
                ...
                familyNumber: {
                    element: familyNumberInput,
                    errorMsg: "Family number is required",
                    validator: (value) => {
                        if (!hasFamilyNumberCheckbox.checked) return true; // Not required if checkbox is unchecked
                        if (!value.trim()) return false;
                        return /^[A-Za-z0-9-]{1,50}$/.test(value.trim());
                    },
                    customErrorMsg: (value) => {
                        if (!hasFamilyNumberCheckbox.checked) return "";
                        if (!value.trim()) return "Family number is required";
                        return "Family number can only contain letters, numbers, and hyphens, up to 50 characters";
                    }
                },
                phone: {
            ...
            // Add "required" attribute dynamically based on checkbox
            function updateFamilyNumberRequired() {
                if (hasFamilyNumberCheckbox.checked) {
                    familyNumberInput.setAttribute("required", "true");
                } else {
                    familyNumberInput.removeAttribute("required");
                }
            }

            // Toggle family number input visibility
            hasFamilyNumberCheckbox.addEventListener('change', function () {
                familyNumberContainer.style.display = this.checked ? 'flex' : 'none';
                updateFamilyNumberRequired();
                if (!this.checked) {
                    familyNumberInput.value = ''; // Clear input when hidden
                    removeFieldError(familyNumberInput);
                }
            });

            // Initialize family number input visibility and required state
            updateFamilyNumberRequired();
            ...
            function populateReviewStep() {
                ...
                document.getElementById("reviewFamilyNumber").textContent = hasFamilyNumberCheckbox.checked ? document.querySelector('input[name="familyNumber"]').value : 'Not provided';
            ...
            ['email', 'phone'].forEach(fieldName => {
                ...
                if (field.element) {
                    field.element.addEventListener('blur', async function() {
                        if (fieldName === 'familyNumber' && !hasFamilyNumberCheckbox.checked) {
            ...
            Object.keys(formFields).forEach(fieldName => {
                if (fieldName !== 'email' && fieldName !== 'phone' && fieldName !== 'familyNumber' && formFields[fieldName].element) {
